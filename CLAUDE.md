...
On the first interaction of a new Claude Code session (NOT on /clear), check whether the `supabase/postgres` image tag used in this repo matches the one in the `~/locaweb-cloud-deploy` repo. That repo has a GitHub Actions workflow that auto-bumps the tag weekly from Docker Hub.

Tell the user you are running this check. Procedure:
1. Search `~/locaweb-cloud-deploy` for the current `supabase/postgres` version tag.
...
5. If `~/locaweb-cloud-deploy` is not available, tell the user and skip.
